        ...
        <div class="flex flex-row justify-between items-center mb-4">
            <h1 class="text-2xl font-semibold mb-4">Document List</h1>
                <x-dropdown name="sort" id="sort" class="text-gray-700 text-sm rounded-md ">
                    ...
                    <x-slot name="content">
                            <x-dropdown-link href="{{ route('document.index', ['sort_by' => 'created_at', 'sort_order' => 'desc']) }}">Created (Newest)</x-dropdown-link>
                            <x-dropdown-link href="{{ route('document.index', ['sort_by' => 'created_at', 'sort_order' => 'desc']) }}">Created (Newest)</x-dropdown-link>
                            <x-dropdown-link href="{{ route('document.index', ['sort_by' => 'created_at', 'sort_order' => 'desc']) }}">Title A-Z</x-dropdown-link>
                            <x-dropdown-link href="{{ route('document.index', ['sort_by' => 'created_at', 'sort_order' => 'desc']) }}">Title A-Z</x-dropdown-link>
                    </x-slot>
                </x-dropdown>
        </div>
